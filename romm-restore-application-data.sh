...
BACKUP_PATH="/srv/romm-application-data/backups/"
RESTORE_PATH="/romm/resources/"
BACKUP_PREFIX="romm-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$ROMM_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$ROMM_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
